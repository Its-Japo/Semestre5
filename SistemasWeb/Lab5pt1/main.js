...
function animar(elem, distancia){
    elem.style.opacity = ((50 + distancia)/50)
    elem.style.transform = "translateY(" + distancia + "px)"
    if (distancia < 0) {
        requestAnimationFrame(function () {
            animar(elem, distancia + 2)
        })
    }
}
...
boton.addEventListener("click", (event) => {
    ...
    temp.class = "mensajeEnviado"
    temp.style.backgroundColor = colorCaja
    temp.style.margin = "8px"
    temp.style.padding = "2px"
    temp.style.paddingLeft = "12px"
    temp.style.border = "1px solid #00aeff"
    temp.class = "mensajeInChat"
    temp.style.color = "white"
    userName = doc.createElement("span")
    userName.style.fontWeight = "bold"
    userName.textContent = "Joaquín: "
    userName.style.color = "#a200ff"
    br = doc.createElement("br")
    mensajeTexto = doc.createElement("p")
    mensajeTexto.style.color = colorTexto
    mensajeTexto.innerHTML = `Mensaje: ${data}`
    temp.appendChild(userName)
    temp.appendChild(br)
    temp.appendChild(mensajeTexto)
    mensajes.push(temp)
    
    for (let i = 0; i < mensajes.length; i++) {
        console.log(temp)
        mensajesBox.appendChild(mensajes[i])
    }
    animar(temp, -50)
